...
var smallIcon = L.icon({
	iconUrl: '/js/leaflet/images/editmarker.png'
	, iconSize: [11, 11]
	, iconAnchor: [6, 6]
});
var bigIcon = L.icon({
    ...
    _addMethods: function() {
        ...
        this.getPoints = function() {
            return this._markers;
				...
				this.clearAddon = function() {
					if (multiopt !== undefined) {
						multiopt.clearLayers();
					}
					for (var i = 0, len = decorators.length ; i < len ; i++) {
						map.removeLayer(arrowPoints[i]);
        ...
        this._parseOptions = function(options) {
            ...
            if(!options.pointIcon)
                //this._options.pointIcon = L.icon({ iconUrl: '/js/leaflet/images/marker-icon.png', iconSize: [25, 41], iconAnchor: [13, 41] });
                //this._options.pointIcon = L.icon({ iconUrl: '/js/leaflet/images/marker-icon.png', iconSize: [13, 21], iconAnchor: [7, 21] });
            ...
            if(!options.newPointIcon)
                this._options.newPointIcon = L.icon({
									iconUrl: '/js/leaflet/images/editmarker2.png'
									, iconSize: [0, 0]
									, iconAnchor: [1, 1] 
								});
        };
        ...
        this._reloadPolyline = function(fixAroundPointNo) {
						...
						multiopt = L.multiOptionsPolyline(that._getMarkerLatLngs(), {
								multiOptions: {
									optionIdxFn: 
									  function (latLng, prevLatLng, index) {
										var pos = (index-1) + ((currentPage - 1)*pagingCount);
										var lum = docslist.lum[pos];
										var idx;
										if (lum < 25.0) {
										  idx = 0;
										} else if (lum < 40.0) {
            ...
            that._showBoundMarkers();
						...
						/**
						 * leaflet - polyline decoration
						 */
						var points = that._getMarkerLatLngs();
						clearDeco();
						for (var d = 0, dLen = (points.length - 1) ; d < dLen ; d++) {
              setArrow(points[d], points[d+1]);
            } 

						function setArrow(pointA, pointB) {
              var arrows = [];
              arrows.push( pointA );
              arrows.push( pointB );
              setArrowHead(arrows);
            ...
            function setArrowHead(arrowPos) {
                var arrow = L.polyline(arrowPos, {stroke:false}).addTo(map);

                var enableStroke = true;
                if (arrowPos[0].equals(arrowPos[1])) {
                    ...
                    patterns: [
                      { offset: '100%',
                        symbol: L.Symbol.arrowHead({
                          ...
                          pathOptions: {stroke: enableStroke, color: '#FF3300', weight: 3}})
                      }
                    ]
                }).addTo(map);
                arrowPoints.push(arrow);
                decorators.push(arrowHead);
            }
        }
        ...
        this._addMarkers = function(pointNo, latLng, fixNeighbourPositions) {
            ...
            marker.on(that._options.addFirstLastPointEvent, function(event) {
                ...
                var marker = event.target;
                var pointNo = that._getPointNo(event.target)+1;
                //console.log('pointNo=' + pointNo + ', page = ' + currentPage);
								var idx = (currentPage*pointNo) - 1;
								ViewinDetail(docslist.id[idx]);
            ...
            marker.on('mouseover', function(event) {
								if (selectedMarkerPoint != null) {
									that._markers[selectedMarkerPoint].setIcon(smallIcon);
								}

                var pointNo = that._getPointNo(event.target);
								var idx = pointNo + ((currentPage - 1)*pagingCount);
                //console.log('pointNo=' + pointNo + ', page = ' + currentPage);
								var trid = docslist.id[idx];
								ViewinDetail(trid);

								$("table tr").removeAttr( 'style' );
								$(".ui-selectee + .ui-selected ").attr("class","ui-selectee")
								$("#"+trid).css("background","lightBlue");

								selectedMarkerPoint = pointNo;
                var marker = event.target;
